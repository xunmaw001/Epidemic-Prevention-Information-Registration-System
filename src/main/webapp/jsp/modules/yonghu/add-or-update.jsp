                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input name="file" type="file" id="shenfenzhengZPhotoupload"
                                               class="form-control-file">
                                        <input name="shenfenzhengZPhoto" id="shenfenzhengZPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>身份证反面</label>
                                        <img id="shenfenzhengFPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="shenfenzhengFPhotoupload"
                                               class="form-control-file">
                                        <input name="shenfenzhengFPhoto" id="shenfenzhengFPhoto-input" type="hidden">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#shenfenzhengZPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('shenfenzhengZPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('shenfenzhengZPhoto-input').setAttribute('value',photoUrl);
            }
        });

        $('#shenfenzhengFPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('shenfenzhengFPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('shenfenzhengFPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                shenfenzhengZPhoto: "required",
                shenfenzhengFPhoto: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                ...
                sexTypes: "性别不能为空",
                yonghuPhoto: "照片不能为空",
                shenfenzhengZPhoto: "身份证正面不能为空",
                shenfenzhengFPhoto: "身份证反面不能为空",
            }
        }).form();
    }
